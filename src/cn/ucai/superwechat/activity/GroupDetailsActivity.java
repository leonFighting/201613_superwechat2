		...
		return new Response.Listener<Message>() {
			...
			public void onResponse(Message message) {
				if (message.isResult()) {
					...
					progressDialog.dismiss();
					Utils.showToast(GroupDetailsActivity.this, Utils.getResourceString(GroupDetailsActivity.this, message.getMsg()), Toast.LENGTH_LONG);
				}
			}
		};
	}
...
//				button.setCompoundDrawablesWithIntrinsicBounds(0, R.drawable.smiley_add_btn, 0, 0);
				...
				Log.e(TAG, "普通item，显示群组成员 name="+member.getMMemberUserName());
				convertView.setVisibility(View.VISIBLE);
...
//				UserUtils.setUserAvatar(mContext, username, holder.imageView);
				...
				button.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							deleteMembersFromGroup(member.getMMemberUserName());
						} else {
		...
		@Override
		public int getCount() {
			return mMembers!=null?mMembers.size()+2:2;
		}
		...
		public void addAll(ArrayList<Member> members) {
			Log.e(TAG,"addAll,members="+members);
	...
	private void registerGroupMemberUpdate() {
		...
		IntentFilter filter = new IntentFilter("update_group_member");
