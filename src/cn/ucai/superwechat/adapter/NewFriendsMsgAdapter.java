...
//			holder.name.setText(msg.getFrom());
                ...
                try {
                    if (msg.getGroupId() == null) { //同意好友请求
                        ...
                        ((NewFriendsMsgActivity) context).executeRequest(new GsonRequest<Group>(path, Group.class,
                                responseAddMemberListener(button, msg), ((NewFriendsMsgActivity) context).errorListener()));
                    }
    ...
    private Response.Listener<Group> responseAddMemberListener(final Button button, final InviteMessage msg) {
                ...
                if (group != null && group.isResult()) {
                    ...
                    try {
                        ...
                        ((Activity) context).runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                pd.dismiss();
                                button.setText(context.getResources().getString(R.string.Has_agreed_to));
                                msg.setStatus(InviteMessage.InviteMesageStatus.AGREED);
                                // 更新db
                                ContentValues values = new ContentValues();
                                values.put(InviteMessgeDao.COLUMN_NAME_STATUS, msg.getStatus().ordinal());
